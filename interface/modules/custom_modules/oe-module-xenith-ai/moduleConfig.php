...
use Xenith\OpenEMR\Modules\TeleHealthModule\AutomationGlobalConfig;
...
if (!empty($_GET['setup']) ?? null) {
     ...
     * $args = ['ctsiVBData' => [
     * 'registrationUri' => 'https://sginsts.XENITHtelehealth.io:28749/CTSIVB/',
     * 'videoApiUri' => 'https://sginsts.XENITHtelehealth.io:22528',
    ...
    $items[TelehealthGlobalConfig::XENITH_VIDEO_REGISTRATION_API] = $credentials['registrationUri'] ?? '';
    $items[TelehealthGlobalConfig::XENITH_VIDEO_TELEHEALTH_API] = $credentials['videoApiUri'] ?? '';
    $items[TelehealthGlobalConfig::XENITH_VIDEO_API_USER_ID] = $credentials['ctsiOrgUid'] ?? '';
    $items[TelehealthGlobalConfig::XENITH_VIDEO_API_USER_PASSWORD] = isset($credentials['ctsiOrgPwd']) ? $cryptoGen->encryptStandard(trim($credentials['ctsiOrgPwd'])) : '';
    $items[TelehealthGlobalConfig::XENITH_VIDEO_TELEHEALTH_CMS_ID] = $credentials['ctsiOrgId'] ?? '';
    $items[TelehealthGlobalConfig::XENITH_TELEHEALTH_PAYMENT_SUBSCRIPTION_ID] = $credentials['paypal_subscription_id'] ?? '';
